...
public class HomeSteps {
        ...
        try {
            ...
            try {
                ...
                for (org.openqa.selenium.WebElement e : driver.findElements(closeBtn)) {
                    try {
                        if (e.isDisplayed() && e.isEnabled()) e.click();
                    } catch (Exception ignored) {
                    }
                }
            } catch (Exception ignored) {
            ...
            try {
                ...
                for (org.openqa.selenium.WebElement b : driver.findElements(backdrop)) {
                    try {
                        if (b.isDisplayed()) {
                            new org.openqa.selenium.interactions.Actions(driver).moveToElement(b).click().perform();
                            Thread.sleep(200);
                        }
                    } catch (Exception ignored) {
                    }
    ...
    /**
     * Robust banner check:
     * - tries many selectors
     * - tries large images count
     * - tries rendered-area heuristic (JS)
     * - logs which selector/heuristic matched
     * - writes failed_home_banners.html when nothing matched
     */
