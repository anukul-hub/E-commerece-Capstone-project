...
public class BookPurchaseSteps {
    ...
    public void i_add_the_book_to_cart() {
        ...
        if (addBtn == null || driver.getCurrentUrl().toLowerCase().contains("/events/") || driver.getCurrentUrl().toLowerCase().contains("festival")) {
            try {
                String query = "system design by alex wu";
                try {
            ...
            try {
                ...
                if (fmt != null) {
                    ...
                    try {
                        ...
                        ((JavascriptExecutor) driver).executeScript("arguments[0].click();", fmt);
                    }
        ...
        try {
            ...
            try {
                ...
                Assert.fail("Failed to click Add/Buy button: " + ex.getMessage());
                return;
            }
        }
        ...
        try {
            ...
            for (WebElement block : addressBlocks) {
                try {
                    ...
                    try {
                        clickable = block.findElement(By.xpath(".//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'use this address') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'deliver') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'ship here') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'select')]"));
                    } catch (Exception ignored) {
                    }
                    if (clickable == null) {
                        try {
                            clickable = block.findElement(By.xpath(".//a[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'deliver') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'ship here') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), 'use this address')]"));
                        } catch (Exception ignored) {
                        }
                    }
                    if (clickable == null) {
            ...
            for (WebElement el : allClicks) {
                try {
                    String txt = "";
                    try {
                        txt = el.getText().trim().toLowerCase();
                    } catch (Exception ignored) {
                    }
